...
class KsnFileManagerAccessorsTest {
    ...
    @Test
    fun `readAll should throw CannotReadKsnFileException when storage read fails`() {
        // Create a stub storage that throws on read
        val failingStorage = object : IPersistentStorage {
        ...
        // Create KsnFileManager with failing storage
        val ksn = object : KsnFileManager(failingStorage) {}

        // Verify that readAll() throws CannotReadKsnFileException
        assertThrows(KsnFileManager.CannotReadKsnFileException::class.java) {
            ksn.readAll()
        }
    }
}
